...
# - 2 indicates the cells visited by the agent while moving around in the house

lut_top_down_map = np.array([[150, 150, 150] for i in range(256)], dtype=np.uint8)


def _bgr_walk(ratio):
    ratio = min(max(ratio, 0.0), 1.0)
    if ratio < 0.5:
        g = round(2 * ratio * 255.0)
        b = 255 - g
        r = 0
    else:
        r = round(2 * (ratio - 0.5) * 255.0)
        g = 255 - r
        b = 0
...
def b_to_r(ratio):
    ratio = min(max(ratio, 0.0), 1.0)
    if ratio < 0.5:
        rgb = 2.0 * (0.5 - ratio) * np.array(
            [0.0, 0.0, 255.0]
        ) + 2.0 * ratio * 255.0 * np.ones(3)
    else:
        rgb = 2.0 * (ratio - 0.5) * np.array([255.0, 0.0, 0.0]) + 2.0 * (
            1.0 - ratio
        ) * 255.0 * np.ones(3)
...
lut_top_down_map[10:] = np.array(
    [_bgr_walk(i / 245) for i in range(0, 246)], dtype=np.uint8
)


lut_top_down_map[0] = [255, 255, 255]  # - 0 means the grid is not visitable
lut_top_down_map[2] = [50, 50, 50]  # - 2 visitable grid border
lut_top_down_map[3] = [255, 0, 255]  # - 2 backtrack!
lut_top_down_map[4] = [0, 0, 200]  # - 4 is the starting position of agent
lut_top_down_map[6] = [200, 0, 0]  # - 6 is target point
