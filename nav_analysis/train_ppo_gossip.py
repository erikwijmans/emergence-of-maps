...
from nav_analysis.gossip_module import SlowMoGossipOptim
...
def init_distrib():
    ...
    dist.init_process_group(
        dist.Backend.MPI, store=tcp_store, rank=world_rank, world_size=world_size
    )
...
def main():
    ...
    with construct_envs(args) as envs:
        ...
        slowmo_freq = 25
        gossip_freq = args.ppo.ppo_epoch * args.ppo.num_mini_batch
        optimizer = SlowMoGossipOptim(
            params,
            base_optim,
            gossip_freq=gossip_freq,
            tau=args.ppo.ppo_epoch * args.ppo.num_mini_batch * slowmo_freq,
        )

        args.logging.reward_window_size = int(
            args.logging.reward_window_size / slowmo_freq
        )
        ...
        with (
            ...
            t_start = time()
            for update in range(update_start_from, args.ppo.num_updates, slowmo_freq):
                losses = torch.tensor(0.0, device=device, dtype=torch.float32)
                step_delta = torch.tensor(0, device=device)

                if world_rank == 0:
                    for it in range(slowmo_freq):
                        is_done_store.set(str(it), "0")
                ...
                for it in range(slowmo_freq):
                    ...
                    for step in range(args.ppo.num_steps):
                        ...
                        if (step + 1) >= (args.ppo.num_steps / 4) and int(
                            is_done_store.get(str(it))
                        ) >= (world_size * args.ddppo.sync_frac):
                            break

                    is_done_store.add(str(it), 1)
                ...
                if world_rank == 0:
                    ...
                    # log stats
                    if True or update > 0 and update % args.logging.log_interval == 0:
                        logger.info(
