...
from .utils import flatten_tensors, make_logger, unflatten_tensors
...
class BilatGossipDataParallel(Module):
    ...
    def _gossip_target(
        ...
        with torch.no_grad():
            ...
            logger.debug(
                "init rcvd: gossip_params {}, gossip_grads {}".format(
            ...
            gossiper = BilatPushPull(
                flatten_tensors(gossip_params),
                graph=graph,
                mixing=mixing,
                logger=logger,
            )
            ...
            while True:
                ...
                try:
                    ...
                    if completed:
                        with gossip_lock:
                            for p, g in zip(
                                gossip_params, unflatten_tensors(in_msg, gossip_params)
                            ):
