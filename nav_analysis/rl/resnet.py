...
class _ShakeShake(Function):
    @staticmethod
    def forward(ctx, x, b1, b2, training=True, mode="shake"):
        ctx.training = training
        ctx.mode = mode
        if training:
            alpha = torch.rand(x.size(0), device=x.device, dtype=x.dtype).view(
                x.size(0), *([1] * (len(x.size()) - 1))
            )
            return x + alpha * b1 + (1.0 - alpha) * b2
        else:
            return x + 0.5 * b1 + 0.5 * b2

    @staticmethod
    def backward(ctx, grad_out):
        if not ctx.training or ctx.mode == "even":
            return grad_out, 0.5 * grad_out, 0.5 * grad_out, None, None
        else:
            x = grad_out
            beta = torch.rand(x.size(0), device=x.device, dtype=x.dtype).view(
                x.size(0), *([1] * (len(x.size()) - 1))
            )
            return x, beta * x, (1.0 - beta) * x, None, None


shake_shake = _ShakeShake.apply


class ShakeShake(nn.Module):
    def forward(self, x, b1, b2):
        return shake_shake(x, b1, b2, self.training)
...
def resneXt25(in_channels, base_planes, ngroups):
    model = ResNet(
        in_channels,
        base_planes,
        ngroups,
...
def se_resneXt25(in_channels, base_planes, ngroups):
    model = ResNet(
        in_channels,
        base_planes,
        ngroups,
...
def shake_shake_resnet50(in_channels, base_planes, ngroups):
    model = ResNet(
        in_channels, base_planes, ngroups, TwoBranchShakeBottleneck, [3, 4, 6, 3]
    )
